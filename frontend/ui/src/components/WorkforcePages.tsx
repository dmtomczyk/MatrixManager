...
export function EmployeesPage({ flash = '' }: PageProps) {
  ...
  const employeeTree = React.useMemo(() => {
    ...
    for (const employee of data.employees) {
      const key = employee.manager_id ?? null;
      const group = byManager.get(key) ?? [];
      group.push(employee);
      byManager.set(key, group);
    }
    for (const group of byManager.values()) group.sort((a, b) => a.name.localeCompare(b.name));
    const ordered: Array<{ employee: Employee; depth: number }> = [];
    const visit = (managerId: number | null, depth: number) => {
      for (const employee of byManager.get(managerId) ?? []) {
        ordered.push({ employee, depth });
        visit(employee.id, depth + 1);
      }
    };
    visit(null, 0);
    return ordered;
  }, [data.employees]);
  const [expandedEmployeeIds, setExpandedEmployeeIds] = React.useState<number[]>([]);
  const visibleEmployees = React.useMemo(() => employeeTree.filter(({ employee, depth }) => {
    if (depth === 0) return true;
    let currentManagerId = employee.manager_id ?? null;
    while (currentManagerId != null) {
      if (!expandedEmployeeIds.includes(currentManagerId)) return false;
      currentManagerId = data.employees.find((candidate) => candidate.id === currentManagerId)?.manager_id ?? null;
    }
    return true;
  }), [employeeTree, expandedEmployeeIds, data.employees]);
  if (loading) return <LoadingState />; if (error) return <ErrorState message={error} />;
  return <PageFrame title="Employees" description="Designed for larger workforces: dense row summaries, visible utilization, and collapsed-by-default reporting chains." flash={flash} stats={stats}><TwoPanel form={<FormCard title="Create employee" description="The table stays compact while the create form remains full-featured."><form className="grid gap-4" onSubmit={async (e) => { e.preventDefault(); try { await sendJson('/employees', 'POST', { ...form, job_code_id: Number(form.job_code_id), organization_id: Number(form.organization_id), manager_id: form.manager_id ? Number(form.manager_id) : null, capacity: Number(form.capacity) }); setForm({ name: '', job_code_id: '', organization_id: '', manager_id: '', role: '', location: '', capacity: '1' }); await reload(); } catch (err) { setError(err instanceof Error ? err.message : 'Failed'); } }}><div className="grid gap-2"><Label>Name</Label><Input value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} required /></div><SelectField label="Job code" value={form.job_code_id} onChange={(value) => setForm({ ...form, job_code_id: value })} options={data.jobCodes.map((code) => ({ value: String(code.id), label: code.name }))} /><SelectField label="Organization" value={form.organization_id} onChange={(value) => setForm({ ...form, organization_id: value })} options={data.organizations.map((org) => ({ value: String(org.id), label: org.name }))} /><SelectField label="Manager" value={form.manager_id} onChange={(value) => setForm({ ...form, manager_id: value })} options={[{ value: '', label: 'No manager' }, ...data.employees.filter((employee) => employee.employee_type === 'L').map((employee) => ({ value: String(employee.id), label: employee.name }))]} required={false} /><div className="grid gap-2"><Label>Role label</Label><Input value={form.role} onChange={(e) => setForm({ ...form, role: e.target.value })} /></div><div className="grid gap-2"><Label>Location</Label><Input value={form.location} onChange={(e) => setForm({ ...form, location: e.target.value })} /></div><div className="grid gap-2"><Label>Capacity</Label><Input type="number" min="0.1" step="0.1" value={form.capacity} onChange={(e) => setForm({ ...form, capacity: e.target.value })} /></div><Button type="submit">Create employee</Button></form></FormCard>} table={<DataCard title="Employees" description="Collapsed by default. Expand leaders to inspect reporting chains."><div className="mb-3 flex flex-wrap gap-2"><Button size="sm" variant="outline" onClick={() => setExpandedEmployeeIds(data.employees.filter((employee) => data.employees.some((candidate) => candidate.manager_id === employee.id)).map((employee) => employee.id))}>Expand All</Button><Button size="sm" variant="outline" onClick={() => setExpandedEmployeeIds([])}>Collapse All</Button></div><CompactTable minWidth="1120px" headers={[{ label: 'Employee', className: 'w-[24%]' }, { label: 'Role / Type', className: 'w-[16%]' }, { label: 'Organization', className: 'w-[15%]' }, { label: 'Manager', className: 'w-[15%]' }, { label: 'Allocation', className: 'w-[8%]' }, { label: 'Status', className: 'w-[8%]' }, { label: 'Actions', className: 'w-[14%]' }]}>{visibleEmployees.map(({ employee, depth }) => <EmployeeRow key={employee.id} employee={employee} depth={depth} isExpanded={expandedEmployeeIds.includes(employee.id)} canExpand={data.employees.some((candidate) => candidate.manager_id === employee.id)} onToggleExpand={() => setExpandedEmployeeIds((prev) => prev.includes(employee.id) ? prev.filter((id) => id !== employee.id) : [...prev, employee.id])} data={data} onReload={reload} onError={setError} />)}</CompactTable></DataCard>} /></PageFrame>;
}

function EmployeeRow({ employee, depth, isExpanded, canExpand, onToggleExpand, data, onReload, onError }: { employee: Employee; depth: number; isExpanded: boolean; canExpand: boolean; onToggleExpand: () => void; data: WorkspaceData; onReload: () => Promise<void>; onError: (value: string) => void }) {
  const [open, setOpen] = React.useState(false);
