      ...
      <table className="w-full min-w-[980px] table-fixed text-sm">
        <thead className="sticky top-0 z-10 bg-slate-50">
          <tr className="border-b border-slate-200 text-left text-[11px] uppercase tracking-[0.14em] text-slate-500">
            {headers.map((header) => <th key={header} className="px-3 py-2.5 font-semibold">{header}</th>)}
          </tr>
        </thead>
...
function CellMain({ title, subtitle }: { title: React.ReactNode; subtitle?: React.ReactNode }) {
  return <div className="min-w-0"><div className="truncate font-medium text-slate-950">{title}</div>{subtitle ? <div className="truncate text-xs text-slate-500">{subtitle}</div> : null}</div>;
}
...
function OrganizationRow({ org, onReload, onError }: { org: Org; onReload: () => Promise<void>; onError: (value: string) => void }) {
  ...
  return <tr className="border-b border-slate-100 align-top"><td className="px-3 py-2.5"><CellMain title={org.name} subtitle={`ID ${org.id}`} /></td><td className="px-3 py-2.5 text-sm text-slate-600">{org.description || '—'}</td><td className="px-3 py-2.5 text-sm text-slate-700">{org.child_organization_count ?? 0}</td><td className="px-3 py-2.5 text-sm text-slate-700">{org.employee_count ?? 0}</td><td className="px-3 py-2.5"><EditorShell open={open} onToggle={() => setOpen((v) => !v)} onDelete={async () => { try { await sendJson(`/organizations/${org.id}`, 'DELETE'); await onReload(); } catch (err) { onError(err instanceof Error ? err.message : 'Failed'); } }}><div className="grid gap-3 xl:grid-cols-[minmax(0,1fr)_minmax(220px,1fr)_auto] xl:items-end"><Input value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} /><Textarea className="min-h-20" value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })} /><Button size="sm" onClick={async () => { try { await sendJson(`/organizations/${org.id}`, 'PUT', form); await onReload(); setOpen(false); } catch (err) { onError(err instanceof Error ? err.message : 'Failed'); } }}>Save</Button></div></EditorShell></td></tr>;
}
