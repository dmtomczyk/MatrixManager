...
export function renderAppChrome(currentUser: string, currentPath: string) {
  ...
  return {
    ...
    css: `
      ...
      @media (max-width: 900px){.nav-links-desktop{display:none}.hamburger-menu{display:block}.nav-links-mobile{display:flex}.account-menu-copy{display:none}.brand-subtitle{display:none}.nav-dropdown-panel-mobile{position:static;min-width:0;border:0;box-shadow:none;padding:6px 0 0}}
    `,
    html: `<header class="app-header"><div class="app-header-inner"><nav class="app-nav" aria-label="Primary"><div class="app-nav-main"><a href="/" class="brand-link"${currentPath === '/' ? ' aria-current="page"' : ''}><img src="/static/images/matrix-manager-favicon.svg" alt="" class="brand-logo" /><span class="brand-copy"><span class="brand-title">Matrix Manager</span><span class="brand-subtitle">Planning Workspace</span></span></a><div class="nav-links nav-links-desktop">${linkMarkup}${planningMarkup}${workforceMarkup}${adminMarkup}</div><details class="hamburger-menu"><summary class="hamburger-trigger" aria-label="Open navigation menu"><span class="hamburger-icon" aria-hidden="true"></span><span>Menu</span></summary><div class="hamburger-panel"><div class="nav-links nav-links-mobile">${linkMarkup}${mobilePlanning}${mobileWorkforce}${mobileAdmin}</div></div></details></div><details class="account-menu"><summary class="account-menu-trigger"><span class="account-icon" aria-hidden="true">👤</span><span class="account-menu-copy"><span class="account-menu-label">Signed in as</span><span class="account-menu-username">${escapeHtml(currentUser)}</span></span></summary><div class="account-menu-panel"><div class="account-menu-meta"><div class="account-menu-meta-top"><span class="account-icon account-icon-panel" aria-hidden="true">👤</span><div><span class="account-menu-label">Signed in as</span><strong>${escapeHtml(currentUser)}</strong></div></div></div><a href="/inbox" class="account-menu-link">Inbox</a><a href="/account-settings" class="account-menu-link">Account Settings</a><a href="/users" class="account-menu-link">Users</a><a href="/audit" class="account-menu-link">Audit</a><a href="/runtime" class="account-menu-link">Runtime</a><a href="/db-management" class="account-menu-link">DB Management</a><form method="post" action="/logout" class="logout-form"><button type="submit" class="logout-button">Logout</button></form></div></details></nav></div></header>`
  };
}
