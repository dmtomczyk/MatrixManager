...
assignmentTable.addEventListener('click', async (event) => { const btn = event.target.closest('button[data-action]'); if (!btn) return; const { action, id } = btn.dataset; if (action === 'edit-assignment') populateAssignmentForm(id); if (action === 'refresh-approvers') { await apiFetch(`/assignments/${id}/refresh-approvers`, { method: 'POST' }); showToast('Approvers refreshed'); await loadAssignments(); } if (action === 'delete-assignment') { if (!confirm('Remove this assignment?')) return; await apiFetch(`/assignments/${id}`, { method: 'DELETE' }); showToast('Assignment removed'); await Promise.all([loadAssignments(), loadDemands()]); } });
