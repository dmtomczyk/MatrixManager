...
const renderAssignments = () => {
  assignmentTable.innerHTML = assignments.map((asg) => {
    ...
    const statusClass = asg.status === 'approved' ? 'assignment-status-approved' : asg.status === 'denied' ? 'assignment-status-denied' : asg.status === 'in_review' ? 'assignment-status-in-review' : '';
    return `<tr><td>${escapeHtml(asg.employee_name || String(asg.employee_id))}</td><td>${escapeHtml(asg.project_name || String(asg.project_id))}</td><td>${escapeHtml(asg.demand_title || '—')}</td><td>${escapeHtml(`${asg.start_date} → ${asg.end_date}`)}</td><td><span class="badge">${Math.round(asg.allocation * 100)}%</span></td><td><span class="badge ${statusClass}">${escapeHtml(asg.status || 'approved')}</span></td><td>${escapeHtml(buildPendingApprovalText(asg))}</td><td>${escapeHtml(asg.submitted_by_username || '—')}</td><td>${reviewActions}</td><td>${escapeHtml(asg.notes || '')}</td><td class="actions"><div class="table-action-stack">${rowActions.join('')}</div></td></tr>`;
  }).join('');
};
