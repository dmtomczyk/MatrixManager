...
const renderAssignments = () => {
  assignmentTable.innerHTML = assignments.map((asg) => `<tr><td>${escapeHtml(asg.employee_name || String(asg.employee_id))}</td><td>${escapeHtml(asg.project_name || String(asg.project_id))}</td><td>${escapeHtml(`${asg.start_date} → ${asg.end_date}`)}</td><td><span class="badge">${Math.round(asg.allocation * 100)}%</span></td><td>${escapeHtml(asg.notes || '')}</td><td class="actions"><button type="button" data-action="edit-assignment" data-id="${asg.id}">Edit</button><button type="button" class="secondary" data-action="delete-assignment" data-id="${asg.id}">Delete</button></td></tr>`).join('');
};
const resetForm = () => { assignmentForm.reset(); assignmentForm.querySelector('input[name="entity_id"]').value = ''; assignmentForm.querySelector('button[type="submit"]').textContent = 'Save Assignment'; assignmentForm.start_date.value = formatISODate(new Date()); assignmentForm.end_date.value = formatISODate(new Date()); };
...
const populateAssignmentForm = (id) => { const assignment = assignments.find((a) => a.id === Number(id)); if (!assignment) return; assignmentEmployeeSelect.value = assignment.employee_id; assignmentProjectSelect.value = assignment.project_id; assignmentForm.start_date.value = assignment.start_date; assignmentForm.end_date.value = assignment.end_date; assignmentForm.allocation.value = Math.round(assignment.allocation * 100); assignmentForm.notes.value = assignment.notes || ''; assignmentForm.querySelector('input[name="entity_id"]').value = assignment.id; assignmentForm.querySelector('button[type="submit"]').textContent = 'Update Assignment'; };
const exportAssignmentsCsv = () => { if (!assignments.length) return alert('No assignments to export yet.'); const rows = assignments.map((asg) => [asg.employee_name || `Employee ${asg.employee_id}`, asg.project_name || `Project ${asg.project_id}`, asg.start_date, asg.end_date, Math.round(asg.allocation * 100)]); const csv = [['Employee','Project','Start Date','End Date','Allocation (%)']].concat(rows).map((line) => line.map((value) => `"${String(value ?? '').replace(/"/g, '""')}"`).join(',')).join('\n'); const blob = new Blob([csv], { type: 'text/csv;charset=utf-8;' }); const url = URL.createObjectURL(blob); const link = document.createElement('a'); link.href = url; link.download = `assignments_${formatISODate(new Date())}.csv`; document.body.appendChild(link); link.click(); document.body.removeChild(link); URL.revokeObjectURL(url); };
assignmentForm.addEventListener('submit', async (event) => { event.preventDefault(); const formData = new FormData(assignmentForm); const payload = { employee_id: Number(formData.get('employee_id')), project_id: Number(formData.get('project_id')), start_date: formData.get('start_date'), end_date: formData.get('end_date'), allocation: (Number(formData.get('allocation')) || 0) / 100, notes: formData.get('notes').trim() || null }; const id = formData.get('entity_id'); try { if (id) { await apiFetch(`/assignments/${id}`, { method: 'PUT', body: JSON.stringify(payload) }); showToast('Assignment updated'); } else { await apiFetch('/assignments', { method: 'POST', body: JSON.stringify(payload) }); showToast('Assignment added'); } resetForm(); await loadAssignments(); } catch (err) { alert(err.message); } });
...
assignmentProjectSelect.addEventListener('change', () => { const project = projects.find((p) => p.id === Number(assignmentProjectSelect.value)); if (project?.end_date) assignmentForm.end_date.value = project.end_date; if (!assignmentForm.start_date.value) assignmentForm.start_date.value = formatISODate(new Date()); });
assignmentExportBtn.addEventListener('click', exportAssignmentsCsv);
(async function init() { await Promise.all([loadEmployees(), loadProjects()]); await loadAssignments(); resetForm(); })();
