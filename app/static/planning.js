...
const getAllocationBounds = () => {
  ...
  if (allocationWindow.mode === 'custom' && allocationWindow.start && allocationWindow.end) {
    return { start: normalizeDate(allocationWindow.start), end: normalizeDate(allocationWindow.end) };
  }
  ...
  end.setDate(end.getDate() + ((allocationWindow.days || DEFAULT_WINDOW_DAYS) - 1));
...
const computeAllocationDataset = () => {
  ...
  employees.forEach((emp) => loadMap.set(emp.id, { info: emp, buckets: new Array(dayCount).fill(0) }));
  assignments.forEach((asg) => {
    const entry = loadMap.get(asg.employee_id);
    if (!entry) return;
    let startIdx = Math.floor((new Date(asg.start_date) - windowStart) / DAY_MS);
    let endIdx = Math.floor((new Date(asg.end_date) - windowStart) / DAY_MS);
    if (endIdx < 0 || startIdx > dayCount - 1) return;
    startIdx = Math.max(0, startIdx);
    endIdx = Math.min(dayCount - 1, endIdx);
    for (let i = startIdx; i <= endIdx; i += 1) entry.buckets[i] += asg.allocation;
  });
  return {
    labels,
    datasets: Array.from(loadMap.values()).map(({ info, buckets }, idx) => ({
      label: info.name,
      data: buckets.map((value) => Math.round(((value / (info.capacity || 1)) * 100) * 10) / 10),
...
const renderAllocationChart = () => {
  ...
  allocationChart = new Chart(ctx, {
    ...
    data: {
      ...
      datasets: dataset.datasets.map((series) => ({
        label: series.label,
        data: series.data,
      ...
      })).concat([{ label: '100% capacity', data: new Array(dataset.labels.length).fill(100), borderColor: '#1d4ed8', borderDash: [6, 4], pointRadius: 0, borderWidth: 1.5 }]),
    },
    options: {
      responsive: true,
...
allocationPresetSelect.addEventListener('change', (event) => {
  const value = event.target.value;
  if (value === 'all') allocationWindow = { mode: 'all' };
  else if (value !== 'custom') allocationWindow = { mode: 'preset', days: Number(value) || DEFAULT_WINDOW_DAYS };
  renderAllocationChart();
});
allocationApplyBtn.addEventListener('click', () => {
  if (!allocationStartInput.value || !allocationEndInput.value) return;
  allocationWindow = { mode: 'custom', start: allocationStartInput.value, end: allocationEndInput.value };
  allocationPresetSelect.value = 'custom';
  renderAllocationChart();
});
(async function init() {
  await Promise.all([loadEmployees(), loadProjects()]);
  await loadAssignments();
  projectForm.start_date.value = formatISODate(new Date());
})();
